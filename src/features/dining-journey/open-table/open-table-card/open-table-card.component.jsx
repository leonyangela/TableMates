...
const OpenTableCard = ({ table, onJoinClick }) => {
  const { location, date, time, totalSeats, seatsAvailable, tableVisibility } = table;
    ...
    <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden hover:shadow-md transition">
      <div className="flex flex-col md:flex-row">
        ...
        <div className="flex-1 p-5 md:p-6 flex flex-col justify-between">
          <div>
            <p className="text-sm text-gray-500 mb-1">Community Table</p>
            <h3 className="text-xl md:text-2xl font-semibold">{location?.name || "Unknown venue"}</h3>
          ...
          <div className="flex flex-wrap gap-x-6 gap-y-3 mt-4 text-sm">
            <div>
              ...
              <p className="font-medium">{moment(date).format("ddd, Do MMM")}</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="font-medium">{isApprovalRequired ? "Needs Approval" : "Join Freely"}</p>
            </div>
          </div>
          ...
          <div className="mt-4 flex items-center gap-3">
            ...
            {seatsAvailable <= 2 && (
              <span className="text-xs text-orange-600 font-medium">Only {seatsAvailable} seat{seatsAvailable === 1 ? "" : "s"} left</span>
            )}
          </div>
        </div>
      </div>
